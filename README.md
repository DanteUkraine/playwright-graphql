...
Build in CLI simplifies code generation process to one simple command.
```bash
playwright-graphql --schema schena.gql
```
...
4. Wright GraphQL tests with joy!
...
type `Sdk` that you can use in your Playwright fixture to return type-safe GraphQL client in tests.
...
```ts
import { getClient, Sdk } from '@gql';
```
Pay attention that path `@fixtures/*` allows to resolve each file in directory as module:
```ts
...
```ts
import { test as baseTest, expect, request, APIRequestContext } from '@playwright/test';
import { getClient, Sdk, RequesterOptions, RequestHandler } from '@gql';
...
```
This full example shows how to customize GpaphQL endpoint, type of response and add custom logic to gql api call
...
[Allows advance configuration of your project](docs/legacy-readme).
...
const gqlApiClient: Sdk = getClient(apiContext, { gqlEndpoint: '/api/graphql' }, customRequestHandler);
```
