...
async function main() {
    ...
    if (argv.custom) {
        ...
        const buildOperationsPath = (schema: string) => join(argv.gqlDir, parse(schema).name, 'autogenerated-operations');
        ...
        const gqlFiles: string[] = (schemas.length === operationsPaths.length && operationsPaths.length === (argv.gqlFile as string[]).length) ?
            ...
            schemas.map(schema => join(argv.gqlDir, `${parse(schema).name}.ts`));
