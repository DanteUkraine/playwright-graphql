...
describe('Setup Codegen CLI', () => {
    ...
    test('generate type safe client with "--introspect false" flag', async () => {
        ...
        const codegenContent = await readFile(join(testDir, codegenFile), 'utf8');
        expect(codegenContent).toMatch(new RegExp(`\\sschema:\\s'${schemaFile}',`));
        expect(codegenContent).toMatch(new RegExp(`\\s'${gqlDirectory}\/${customOperations}\/\\*\\*\/\\*.gql'`));
        expect(codegenContent).toMatch(new RegExp(`\\s'${gqlDirectory}\/${gqlFile}':`));
        expect(codegenContent).toMatch(new RegExp(`\\srawRequest:\\sfalse,`));
        ...
        const codegenContent2 = await readFile(join(testDir, codegenFile), 'utf8');
        expect(codegenContent2).toMatch(new RegExp(`\\sschema:\\s'${schemaFile}',`));
        expect(codegenContent2).toMatch(new RegExp(`\\s'${gqlDirectory}\/${customOperations}\/\\*\\*\/\\*.gql'`));
        expect(codegenContent2).toMatch(new RegExp(`\\s'${gqlDirectory}\/${gqlFile}':`));
        expect(codegenContent2).toMatch(new RegExp(`\\srawRequest:\\sfalse,`));
