...
describe('Setup Codegen CLI', () => {
    ...
    const expectedAutogeneratedOperationsGlob = '/autogenerated-operations/**/*.gql';
    const expectedCustomOperationsGlob = '/custom-operations/**/*.gql';
    ...
    test('generates schema and operations when no existing files', async () => {
        ...
        const codegen = await loadCodegenConfig({ configFilePath: path.join(testDir, codegenFile) });
        expect(codegen.config.schema).toEqual(schemaFile)
        expect(codegen.config.documents).toEqual([`${gqlDirectory}${expectedAutogeneratedOperationsGlob}`]);
        expect(codegen.config.generates).toMatchObject({
            [`${gqlDirectory}/${gqlFile}`]: { config: { rawRequest: false } },
        });
    ...
    test('updates existing codegen config if new schema path provided', async () => {
        ...
        expect(codegen.config.schema).toEqual(updatedSchemaFile)
        expect(codegen.config.documents).toEqual([`gql${expectedAutogeneratedOperationsGlob}`]);
        expect(codegen.config.generates).toMatchObject({
            [`gql/graphql.ts`]: { config: { rawRequest: false } },
        });
    ...
    test('skip execution if gql directory or file properties absent in codegen.ts', async () => {
        ...
        expect(cliLogs).toMatchObject({
            stdout: 'Operations were generated and saved to "gql/autogenerated-operations".\n' +
                    'Could not find path \"gql/autogenerated-operations\" to autogenerated operations in \"codegen.ts\".\n' +
                    'Could not find path to autogenerated graphql client file \"gql/graphql.ts\" in \"codegen.ts\".\n' +
                    'Remove file "codegen.ts" to generate new one or fix existing file manually.\n',
        });
    });
    ...
    test('generated type script should contain modification', async () => {
        ...
        expect(codegen.config.documents).toEqual([`${gqlDirectory}${expectedAutogeneratedOperationsGlob}`]);
        expect(codegen.config.generates).toMatchObject({
            [`${gqlDirectory}/${gqlFile}`]: { config: { rawRequest: true } },
        });
    ...
    test('generate type safe client with "--introspect" flag', async () => {
        const cliLogs = await execAsync(
            `node ${cliPath} --url ${stabServer} --schema ${schemaFile} --gqlDir ${gqlDirectory} --gqlFile ${gqlFile} --raw --codegen ${codegenFile} -i`,
            { cwd: testDir }
        );
        ...
        expect(cliLogs).toMatchObject({
            stdout: `Schema generated from "${stabServer}" to "test-schema.gql".\n` +
                `Operations were generated and saved to "${gqlDirectory}/autogenerated-operations".\n` +
                `File "${codegenFile}" generated.\n` +
                `Type Script types for Playwright auto generated type safe GQL client generated.\n`,
        });
    ...
    test('generate type safe client with "--introspect false" flag', async () => {
        const customOperations = 'custom-operations';

        const cliLogs = await execAsync(
            `node ${cliPath} --url ${stabServer} --schema ${schemaFile} --gqlDir ${gqlDirectory} --gqlFile ${gqlFile} --codegen ${codegenFile} --coverage --introspect false`,
            { cwd: testDir }
        );
        ...
        const codegen = await loadCodegenConfig({ configFilePath: path.join(testDir, codegenFile) });
        expect(codegen.config.schema).toEqual(schemaFile)
        expect(codegen.config.documents).toEqual([`${gqlDirectory}${expectedCustomOperationsGlob}`]);
        expect(codegen.config.generates).toMatchObject({
            [`${gqlDirectory}/${gqlFile}`]: { config: { rawRequest: false } },
        });

        const graphqlDir = await readdir(join(testDir, gqlDirectory), { withFileTypes: true });

        expect(graphqlDir.map(i => i.name ).sort()).toEqual([ customOperations ].sort());

        expect(cliLogs).toMatchObject({
            stdout: `Schema generated from "${stabServer}" to "test-schema.gql".\n` +
                'File "gql-codegen.ts" generated.\n' +
                `No operations found, API client "${gqlDirectory}/${gqlFile}" was not generated, add operations and generate again.\n`,
        });

        const customOperationContent =
            'query hello {\n' +
                'hello\n' +
            '}\n';

        await writeFile(join(testDir, `${gqlDirectory}/${customOperations}/operations.gql`), customOperationContent);

        const cliLogs2 = await execAsync(
            `node ${cliPath} --url ${stabServer} --schema ${schemaFile} --gqlDir ${gqlDirectory} --gqlFile ${gqlFile} --codegen ${codegenFile} --coverage --introspect false`,
            { cwd: testDir }
        );

        const dir2 = await readdir(testDir, { withFileTypes: true });
        expect(dir2.map(i => i.name ).sort()).toEqual([ schemaFile, gqlDirectory, codegenFile ].sort());

        const updatedCodegen2 = await loadCodegenConfig({ configFilePath: path.join(testDir, codegenFile) });
        expect(updatedCodegen2.config.schema).toEqual(schemaFile)
        expect(updatedCodegen2.config.documents).toEqual([`${gqlDirectory}${expectedCustomOperationsGlob}`]);
        expect(updatedCodegen2.config.generates).toMatchObject({
            [`${gqlDirectory}/${gqlFile}`]: { config: { rawRequest: false } },
        });

        const graphqlDir2 = await readdir(join(testDir, gqlDirectory), { withFileTypes: true });

        expect(graphqlDir2.map(i => i.name ).sort()).toEqual([ customOperations, gqlFile ].sort());

        expect(cliLogs2).toMatchObject({
            stdout: `Schema generated from "${stabServer}" to "test-schema.gql".\n` +
                    `Type Script types for Playwright auto generated type safe GQL client generated.\n`,
        });
    });
